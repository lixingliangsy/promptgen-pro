...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  // Which billing cycle? Default monthly; use yearly only if a product id is configured.
  const cycle =
    req.query && req.query.cycle === 'yearly' ? 'yearly' : 'monthly'
  const productId = cycle === 'yearly'
    ? process.env.WAFFO_YEARLY_PRODUCT_ID
  ...
  return res.redirect(302, process.env.WAFFO_CHECKOUT_URL || '/#pricing')
}
